...
/*include keys from blob storage*/
#include "blob/blob_keys/private_key.h"
#include "blob/blob_keys/public_key.h"
#include "blob/blob_keys/psk.h"
...
{
    fmt_hex_bytes(credentials->private_key, (const char *)private_key);
    fmt_hex_bytes(credentials->public_key, (const char *)public_key);
    fmt_hex_bytes(credentials->psk_key, (const char *)psk);
}


static void _set_credentials_mem(wot_credentials_t *credentials)
{
...
int main(void)
{
    ...
    _set_credentials_mem(&credentials);
