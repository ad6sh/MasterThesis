...
static bool _set_remote_multicast(sock_udp_ep_t *remote, char *addr_str)
{
    ...
    if (!iface) {
        ...
        if (gnrc_netif_get_by_pid(atoi(iface)) == NULL) {
            printf("[CoAP] interface not valid");
            return false;
        }
    ...
    if (ipv6_addr_from_str(&addr, addr_str) == NULL) {
        printf("[CoAP] unable to parse destination address");
        return false;
    }
    if ((remote->netif == SOCK_ADDR_ANY_NETIF) && ipv6_addr_is_link_local(&addr)) {
        printf("[CoAP] must specify interface for link local target");
        return false;
    }
...
int test_resource_discovery(int argc, char **argv)
{
    (void)argc;
    (void)argv;
    if (argc != 1) {
    ...
    coap_pkt_t pdu;
    size_t len;
    unsigned msg_type = COAP_TYPE_CON; //COAP_TYPE_NON
    ...
    coap_opt_add_uri_query(&pdu, "rt", "test");
    len = coap_opt_finish(&pdu, COAP_OPT_FINISH_NONE);

    sock_udp_ep_t remote;
    if (!_set_remote_multicast(&remote, MULTI_CAST_ADDR)) {
        puts("failed to set remote address");
        return 1;
    }
    size_t bytes_sent = gcoap_req_send(&buf[0], len, &remote, _resp_handler, NULL);
    if (bytes_sent > 0) {
        puts("requested resource discovery");
    }
    else {
        puts("failed to request resource discovery");
        return 1;
    }
    return 0;
