...
int wot_get_cbor_certificate(uint8_t *buf, int type)
{
    ...
    switch (type) {
    case CBOR_RD_CERT:
        common_name = strndup(RD_NAME, strlen(RD_NAME));
        memcpy(pubkey, rd_ecdsa_pub_key_x, 32);
        memcpy(pubkey + 32, rd_ecdsa_pub_key_y, 32);
        break;
    case CBOR_CLI_CERT:
        common_name = strndup(CLIENT_NAME, strlen(CLIENT_NAME));
